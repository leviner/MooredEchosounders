...
for i in [0,1,2]:
    ...
    print('Site C',str(site[i]),'2017-2018 (depth = '+str(y['depth (m)'].unique()[0])+'): \n - T:',round(y['temperature (degree_C)'].mean(),3),'\n - S:',round(y['salinity'].mean(),3),
          '\n - C:',round(seawater.eos80.svel(y['salinity'].mean(),y['temperature (degree_C)'].mean(),y['depth (m)'].unique()[0]),3),
          '(',round(seawater.eos80.svel(y['salinity'],y['temperature (degree_C)'],y['depth (m)'].unique()[0]).min(),3),
          round(seawater.eos80.svel(y['salinity'],y['temperature (degree_C)'],y['depth (m)'].unique()[0]).max(),3),')')
    ...
    y = a[a['time (UTC)'] > pd.to_datetime(dt[i]).tz_localize('UTC')]
    print('Site C',str(site[i]),'2018-2019 (depth = '+str(y['depth (m)'].unique()[0])+'): \n - T:',round(y['temperature (degree_C)'].mean(),3),'\n - S:',round(y['salinity'].mean(),3),
          '\n - C:',round(seawater.eos80.svel(y['salinity'].mean(),y['temperature (degree_C)'].mean(),y['depth (m)'].unique()[0]),3),
          '(',round(seawater.eos80.svel(y['salinity'],y['temperature (degree_C)'],y['depth (m)'].unique()[0]).min(),3),
          round(seawater.eos80.svel(y['salinity'],y['temperature (degree_C)'],y['depth (m)'].unique()[0]).max(),3),')')
